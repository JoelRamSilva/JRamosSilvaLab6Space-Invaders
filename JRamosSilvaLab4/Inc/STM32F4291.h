...
typedef struct{
	...
	uint32_t TIMx_DCR;
	uint32_t TIMx_DMAR;
	uint32_t TIMx_OR;		//Look at manual for both TIM2 and TIM5
}GPTIMR_RegDef_t;

#define TIM2 ((GPTIMR_RegDef_t*) TIM2_BASE_ADDR)
#define TIM5 ((GPTIMR_RegDef_t*) TIM5_BASE_ADDR)
#define TIM2_RCC_OFFSET 0
#define TIM5_RCC_OFFSET	3

#define RCC_TIMx_ENABLE(OFFSET) (RCC->RCC_APB1ENR |= 1 << OFFSET)
#define RCC_TIMx_DISABLE(OFFSET) (RCC->RCC_APB1ENR &= ~(1 << OFFSET))
