...
void TurnOffLED(uint8_t ledNum){
	...
	case RED_LED:
		WriteToOutputPin(GPIOG, redLed.PinNumber, DISABLE);

	case GREEN_LED:
		WriteToOutputPin(GPIOG, greenLed.PinNumber, DISABLE);
	}
}
...
void TurnOnLED(uint8_t ledNum){
	...
	case RED_LED:
		WriteToOutputPin(GPIOG, redLed.PinNumber, ENABLE);

	case GREEN_LED:
		WriteToOutputPin(GPIOG, greenLed.PinNumber, ENABLE);
	}
}
